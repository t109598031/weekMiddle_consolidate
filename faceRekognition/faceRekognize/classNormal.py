...
class Object:
    def __init__(self,dataModel,aws_access_key_id,aws_secret_access_key,sourceBucketName,collectionId,region_name):
        ...
        self.__objBucketName = objBucketName
        self.__objBucketSubPathList = objBucketSubPathList
        ...
        self.__memberFaceFileName = memberFaceFileName + ".json"
    ...
    #待修正Face改obj泛用
    def storeObjectImage(self,objId,imageList,uid):
        objImageUrlList = []
        serialNo = 0 #非註冊模式下儲存人臉的順序
        if self.__dataModel["config"]["rekognize"]["mode"] == "0":
            subPath = self.__objBucketSubPathList[0]
        else:
            subPath = self.__objBucketSubPathList[1]
        for image in imageList:
            ...
            response=self.__s3Client.put_object(ACL='public-read',
                                                Body=image,
                                                Bucket=self.__objBucketName,
                                                Key=fileName,
                                                ContentEncoding='base64',
                                                ContentType='image/jpeg')
            objImageUrlList.append('https://' + self.__objBucketName + '.s3-' + self.__region_name + '.amazonaws.com/' + fileName)
            serialNo += 1
        return objImageUrlList
...
class Registration(Face):
    ...
    def registeredFace(self):
        ...
        for face in faceDetectResponse["FaceDetails"]:
            faceModel = {}
            imageList = self._spliteImage([face["BoundingBox"]])
    ...
    def createFace(self,binaryImage,uid):
        faceIdList = []
...
class SignIn(Face):
    ...
    def signInFace():
        ...
        if len(faceDetectResponse["FaceDetails"]) >= 1: #複數人臉
            ...
            faceImageList = self._spliteImage(faceBoundingBoxList) #切割
